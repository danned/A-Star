...
# Agent strategy 2
def pokerStrategyExampleAggro(playerAction, playerActionValue, playerStack, agentHand, agentHandRank, agentStack):
    ...
    if Type[agentHand] == 1 and Rank[agentHandRank] < Rank['Q']:  # Hand rank is lower than queen
        if playerActionValue < 0.2*playerStack:
            if agentStack < playerActionValue:
                agentAction = 'Bet'
                agentValue = agentStack
            else:
            ...
            if playerActionValue < 0.3*agentStack:
                if agentStack > playerStack:
                    if agentStack < playerActionValue:
                        agentAction = 'Bet'
                        agentValue = agentStack
    ...
    if (Type[agentHand] == 1 and Rank[agentHandRank] >= Rank['Q']) \
            or (Type[agentHand] == 2 and Rank[agentHandRank] <= Rank['T']):
        # print(playerActionValue, type(agentStack))
        if playerActionValue < 0.2*agentStack:
            if agentStack < playerActionValue:
                agentAction = 'Bet'
                agentValue = agentStack
            else:
            ...
            if playerActionValue < 0.4*agentStack:
                if agentStack < 2*playerActionValue:
                    agentAction = 'Bet'
                    agentValue = agentStack
                else:
    ...
    if Type[agentHand] == 2 and Rank[agentHandRank]> Rank['T'] and Rank[agentHandRank] < Rank['A']:
        if playerActionValue < 0.3*playerStack:
            if agentStack < playerStack:
                if playerActionValue < 0.25*agentStack:
                    if agentStack < 2*playerActionValue:
                        agentAction = 'Bet'
                        agentValue = agentStack
                    else:
    ...
    if Type[agentHand] == Type['TwoPairs']:
        if playerActionValue < 0.3*playerStack:
            if Rank[agentHandRank] > Rank['9']:
                if agentStack < 2*playerActionValue:
                    agentAction = 'Bet'
                    agentValue = agentStack
                else:
            ...
            if playerActionValue < 0.2*agentStack:
                if Rank[agentHandRank] > Rank['9'] and agentStack > playerStack:
                    if agentStack < 2*playerActionValue:
                        agentAction = 'Bet'
                        agentValue = agentStack
                    else:
    ...
    if Type[agentHand] == Type['3ofakind']:
        if agentStack > 1.5*playerStack:
            if playerActionValue < 0.35*agentStack:
                if playerActionValue > 0:
                    if agentStack < 2*playerActionValue:
                        agentAction = 'Bet'
                        agentValue = agentStack
    ...
    if Type[agentHand] > Type['3ofakind']:
        if agentStack > 1.5*playerStack:
            if playerActionValue < 0.28*agentStack:
                if playerActionValue > 0:
                    if agentStack < 2*playerActionValue:
                        agentAction = 'Bet'
                        agentValue = agentStack
